...
import {
  useHospitalChatMessages,
  useHospitalChatThreads,
  useMarkHospitalChatRead,
  useSendHospitalChatMessage,
} from '../../hooks/useHospitalChats'
...
function displayTitle(thread: ChatThread) {
  const patient = thread.patient?.fullName || 'Patient'
  const doctor = thread.doctor?.fullName || 'Doctor'
...
function displaySubtitle(thread: ChatThread) {
  const parts = [
    thread.hospitalDoctor?.department?.name,
...
function roleLabel(role?: string) {
  if (role === 'HOSPITAL_STAFF') return 'You'
...
function Badge({ value }: { value?: string | null }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-black ring-1 dark:bg-white/10 dark:text-slate-200 dark:ring-white/10 ${tone}`}
    >
      {value}
    </span>
  )
...
function MessageBubble({ message }: { message: ChatMessage }) {
  const mine = message.senderRole === 'HOSPITAL_STAFF'
  ...
  return (
    <div className={mine ? 'flex justify-end' : 'flex justify-start'}>
      <div
        className={
          mine
            ? 'max-w-[82%] rounded-3xl bg-cyan-600 px-4 py-3 text-white shadow-lg shadow-cyan-600/20'
      ...
      >
        <div className="mb-1 flex items-center justify-between gap-4">
          ...
          <span
            className={
              mine ? 'text-[11px] text-cyan-100' : 'text-[11px] text-slate-400'
            }
          >
...
export function HospitalChatsPage() {
  const q = useHospitalChatThreads()
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Monitor and reply to appointment conversations.
        </p>
      </div>
      ...
      {q.isLoading ? (
        ...
        <Card>
          <p className="text-sm text-slate-500">
            No chats yet. Patients can start chats from appointment details.
          </p>
        </Card>
      )}
    </div>
...
export function HospitalChatDetailPage() {
  const { threadId } = useParams()
  const q = useHospitalChatMessages(threadId)
  const send = useSendHospitalChatMessage(threadId)
  const read = useMarkHospitalChatRead(threadId)
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        <Card className="flex min-h-[68vh] flex-col gap-4">
          ...
          <div className="grid gap-3">
            <textarea
              ...
              placeholder="Write a hospital reply..."
              className="min-h-24 w-full rounded-2xl border border-slate-200/80 bg-white/75 px-4 py-3 text-sm shadow-sm backdrop-blur focus:outline-none focus:ring-2 focus:ring-cyan-300/50 dark:border-white/10 dark:bg-white/10 dark:text-white"
              onKeyDown={(event) => {
        ...
        <Card className="h-fit">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            ...
            <div>
              <dt className="text-slate-500">Patient Email</dt>
              <dd className="break-all font-bold dark:text-white">
                {thread.patient?.user?.email || '—'}
              </dd>
            </div>
