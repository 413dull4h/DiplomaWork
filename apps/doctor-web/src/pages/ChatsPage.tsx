...
function MessageBubble({ message }: { message: ChatMessage }) {
  const mine = message.senderRole === 'DOCTOR'
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Appointment conversations with your patients.
        </p>
      </div>
      ...
      {q.isLoading ? (
        ...
        <Card>
          <p className="text-sm font-bold text-rose-600">
            Could not load doctor chats. Make sure you are logged in as a doctor.
          </p>
        </Card>
      ) : sorted.length ? (
          ...
          <div className="flex-1 space-y-3 overflow-y-auto rounded-3xl bg-slate-50/70 p-3 dark:bg-slate-950/30">
            {messages.length ? (
              ...
              <div className="grid h-full place-items-center text-center">
                <p className="text-sm text-slate-500">
                  No messages yet. Reply when the patient starts the conversation.
                </p>
              </div>
            )}
          </div>
