import { Activity, Building2, CalendarDays, ClipboardList, LayoutDashboard, Settings, UserCircle, Users, HeartPulse } from 'lucide-react'
  ...
  return <aside className="sticky top-0 hidden h-screen w-72 shrink-0 p-4 lg:block">
    <div className="glass-surface flex h-full flex-col rounded-glass p-4">
      ...
      <nav className="space-y-2">
        {items.map(({ to, key, fallback, icon: Icon }) => <NavLink key={to} to={to} className={({ isActive }) => cn('flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition', isActive ? 'bg-primary text-white shadow-glow' : 'text-muted hover:bg-white/50 hover:text-foreground dark:hover:bg-white/10')}><Icon size={18} />{t(key, fallback)}</NavLink>)}
      </nav>
    </div>
  </aside>
}
